...
public class ReviewRepository : IReviewRepository
{
    ...
    public async Task<bool> CustomerAlreadyReviewedTripAsync(int customerId, int tripId)
    {
        return await _context.Reviews.AnyAsync(r =>
            r.CustomerId == customerId && r.TripId == tripId
        );
    }
    ...
    {
        var ratings = _context.Reviews.Where(r => r.TripId == tripId);

        return await ratings.AnyAsync() ? await ratings.AverageAsync(r => (double)r.Rating) : 0;
    }
}